...
int main()
{
	char* str= "124##5##36##";
	char* str2 = "124###35##6";
	BinaryTree<char> tree(str,'#');
	BinaryTree<char> tree2(str2,'#');

	TreeNode<char>* ret = tree.Find('7');
	TreeNode<char>* ret2 = tree.GetParent(ret);
	size_t h = tree.GetLevelNode(4);
	size_t h2 = tree2.GetLevelNode(4);
	tree2.GetBinaryMirror_Nor();
	tree2.PreOrder();
	tree2.GetBinaryMirror_Nor();
	tree2.PreOrder();
	if (tree2.isCompleteBinaryTree())
		cout<<"is "<<endl;
	else
		cout<<"not is"<<endl;
